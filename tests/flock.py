...
# ================================================================
# temp debugging functions for cpp implementation
# ================================================================

def dotProdsForSeed(X, Xblur, seed, windowLen):
	x = X[:, seed:seed+windowLen]
	return _dotProdsWithAllWindows(x, Xblur)
...
def _dotProdsWithAllWindows(x, X):
	...
	# m = x.shape[1]
	# numSubseqs = X.shape[1] - m + 1
	# return np.array([np.sum(x * X[:, i:i+m]) for i in range(numSubseqs)]).flatten()
...
def _findAllInstancesFromSeedLoc(X, Xblur, seedStartIdx, seedEndIdx,
	...
	countsBlur = np.copy(Xblur[:, idx:idx+windowLen]) + .0001
