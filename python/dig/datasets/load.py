...
from ucr import getAllUCRDatasets
...
def loadDataset(datasetName, seed=None, whichExamples=None, instancesPerTs=5,
	...
	if func:
		...
		for i in range(len(whichExamples)):
			(data, start1, start2), m = func(returnStartIdxs=True,
				noise=SYNTHETIC_DATA_NOISE, backgroundNoise=SYNTHETIC_DATA_NOISE)
		...
		for r in recordings:
			...
			if not useMFCCs:
				print("WARNING: TIDIGITS start and end indices only valid for"
					"mfccs (because of a hack--see getConcatenatedRecordingsForDigits)")
	...
	# ucr
		...
		if ucrAll or ucrShort or ucrPairs: # ucr_all | ucr_short | ucr_pairs
			if ucrAll:
				dirs = ucr.getAllUCRDatasetDirs()
			else:
				dirs = ucr.getShortUCRDatasetDirs()
			...
			for dataDir in dirs:
				...
				# if 'Adiac' not in dataDir:
				# 	continue

				tsFromDataset = ucr.labeledTsListFromDataset(dataDir,
					instancesPerTs=instancesPerTs)
				if hasWhichExamples:
					tsFromDataset = [tsFromDataset[i] for i in whichExamples if i < len(tsFromDataset)]
				tsFromDataset = filter(lambda ts: len(ts.labels) >= instancesPerTs, tsFromDataset)
